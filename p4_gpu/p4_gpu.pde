...
void draw() {
  ...
  shader(bumps_shader);
  float offset = 5.0;
  for (int i = -300; i < -50; i += 5) {
    for (int j = 50; j < 300; j += 5) {
      beginShape();
      texture(ship_texture);
      vertex(i, j, -300, (i + 300) / 250.0, (j - 50) / 250.0);
      vertex(i + offset, j, -300, (i + offset + 300) / 250.0, (j - 50) / 250.0);
      vertex(i + offset, j + offset, -300, (i + offset + 300) / 250.0, (j + offset - 50) / 250.0);
      vertex(i, j + offset, -300, (i + 300) / 250.0, (j + offset - 50) / 250.0);
      endShape();
    }
  }
